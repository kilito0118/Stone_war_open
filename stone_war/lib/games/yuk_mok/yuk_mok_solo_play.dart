...
class YukMokSoloPlay extends StatefulWidget {
  const YukMokSoloPlay({super.key});

  @override
  State<YukMokSoloPlay> createState() => _YukMokSoloPlayState();
}

class _YukMokSoloPlayState extends State<YukMokSoloPlay> {
  double _top = -40;
  double _left = -180;
...
//소리 재생 부분
  ...
  void _placeStone(Offset position) {
    // 클릭된 행 계산(한 칸이 40이어서 20은 반이다.)
    int row = ((position.dy - 75 + 20) / cellSize).floor();
